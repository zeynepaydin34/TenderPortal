...
if (isset($_GET['bidId'])) {
    $bidId = $_GET['bidId'];
    // Proceed with handling the tender based on the ID
}
    ...
    <body>
        ...
        if (mysqli_num_rows($result2) > 0) {
            while ($tender = mysqli_fetch_assoc($result2)) {
                ...
                echo '<form action="./offer.php" method="GET">';
                echo '<input type="hidden" name="bidId">';
                echo '<button type="submit" name="submit" id="offer-button" class="offer-btn">TEKLİF VER</button>';
                echo '</form>';
                echo '</li>';
        ...
        mysqli_close($conn); //Veritabanı bağlantısı kapatıldı
        ?>
...
<?php
// Çıktı tamponlamasını sonlandır ve tamponu boşalt
ob_end_flush();
?>
<script>
